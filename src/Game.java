...
//		tr.add(new Point2D.Double(180, 220));
		...
		Entity enemyblock = new Entity(eplist);//(0+2, 0,  windowSize[0]-4, 50+10*level, 0, 0);
...
//		enemyblock.poly.filled = true;
		...
		Entity floor = new Entity(-wallThickness, windowSize[1], wallThickness*2+windowSize[0], wallThickness, 0, 0);
		floor.solid = true;
		floor.moving = false;
		floor.name = "floor";
...
//		floor.elasticity = 1;
		...
		Entity lwall  = new Entity(-wallThickness, -wallThickness, wallThickness, wallThickness*2+windowSize[1], 0, 0);
		lwall.solid = true;
		lwall.moving = false;
		lwall.name = "lwall";
		...
		Entity rwall  = new Entity(windowSize[0], -wallThickness, wallThickness, wallThickness*2+windowSize[1], 0, 0);
		rwall.solid = true;
		rwall.moving = false;
		rwall.name = "rwall";
		...
		Entity ceil  = new Entity(-wallThickness, -wallThickness, wallThickness*2+windowSize[0], wallThickness, 0, 0);
		ceil.solid = true;
		ceil.moving = false;
		ceil.name = "ceil";
		ceil.density = Double.POSITIVE_INFINITY;
		ceil.gravity = 0;
		ceil.collisionCategory = 2;//victory if colliding with 1, regular with anything else
		entities.add(ceil);
		...
		paddle.gravity = 0;
		paddle.baseSpeed = 5;
		paddle.collisionCategory = 5;//lost points on collision with 0. delete the 0
    ...
    public void run() {
        ...
        window.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if(button == 3){
					cutLine = new Line(lastCutPoint.x, lastCutPoint.y, e.getX()+camera.getBounds2D().getX(), e.getY()+camera.getBounds2D().getY());
...
//            	System.out.println("step");
            ...
            /*if(camera.x < 0){
            	camera.x = 0;
            	camera.xAcc = 0;
        	}else if(camera.x > mapBounds[0]-camera.w){
        		camera.x = mapBounds[0]-camera.w;
        		camera.xAcc = 0;
        	}
            if(camera.y < 0){
            	camera.y = 0;
            	camera.yAcc = 0;
        	}else if(camera.y > mapBounds[1]-camera.h){
        		camera.y = mapBounds[1]-camera.h;
        		camera.yAcc = 0;
        	}//*/
...
//                        						score = score + 20 + levelsCleared*10 - penalties;
                        						loss = true;
                        						penalties = 0;
                        						levelsCleared = 0;
//                        						buildLevel(levelsCleared);
...
//                        						buildLevel(levelsCleared);
                        					...
                        					boolean doCut = (!e1.markedForDelete && !e2.markedForDelete && e1.collisionCategory == 1 && e2.collisionCategory == 0);
//                        					if(e1.collisionCategory == 1 && e2.collisionCategory == 5){
//                        						
//                        					}else{
...
//                        						double maxPaddleAngleR = Math.toRadians(maxPaddleAngle);
                        						double paddleMid = paddle.getBounds2D().getX() + paddle.getBounds2D().getWidth()/2;
                        						double wedgeMid = wedge.getBounds2D().getX() + wedge.getBounds2D().getWidth()/2;
                        						double offset = wedgeMid-paddleMid; //how far right of center the wedge is
                        						double percentOffset = offset/ paddle.getBounds2D().getWidth();
                        						wedge.velocity.add(5*percentOffset, 0);
//                        						double adjustedAngle = maxPaddleAngle * percentOffset * 2;
//                        						double angleR = Math.toRadians(adjustedAngle);
//                        						Vector wedgeVel = new Vector(0, 1);
...
////                        						double wedgeBottom = e2.getBounds2D().getY() + e2.getBounds2D().getHeight();
////                        						double paddleTop = paddle.getBounds2D().getY();
////                        						e2.poly.translate(new Vector(0, paddleTop-wedgeBottom-10));
//                        						
//                        						System.out.println("paddle.getBounds2D().getWidth(): " + paddle.getBounds2D().getWidth());
//                        						System.out.println("paddleMid: " + paddleMid);
//                        						System.out.println("wedgeMid: " + wedgeMid);
//                        						System.out.println("offset: " + offset);
//                        						System.out.println("percentOffset: " + percentOffset);
//                        						System.out.println("adjustedAngle: " + adjustedAngle);
//                        						System.out.println("wedgeVel: " + wedgeVel);
                        					}
    		...
    		Line testLine = new Line((float)wedge.getBounds2D().getCenterX(), (float)wedge.getBounds2D().getCenterY(), wedge.velocity);
    		testLines.add(testLine);
    		
//    		Collision testcol = wedge.getCollisionInfo(testP);
//    		System.out.println("collision normal\n x: " + testcol.normal.x + ", y: " + testcol.normal.y );
//    		System.out.println("backtrack\n x: " + testcol.backtrack.x + ", y: " + testcol.backtrack.y );
//    		System.out.println("loc\n x: " + testcol.loc.x + ", y: " + testcol.loc.y );
//    		System.out.println("vertex\n x: " + testcol.vertex.x + ", y: " + testcol.vertex.y );
//    		Line normalL = new Line(testcol.loc.getX(), testcol.loc.getY(), testcol.normal);
//    		normalL.color[0] = .5f;
//    		normalL.color[1] = .5f;
//    		normalL.color[2] = 1f;
//    		testLines.add(normalL);
//    		Line backTrackL = new Line(testcol.vertex.getX(), testcol.vertex.getY(), testcol.backtrack);
...
//    		testLines.add(backTrackL);
    		...
    		/*
    		Collision testcol2 = testP.getCollisionInfo2(wedge, wedge.velocity);
    		System.out.println("collision normal\n x: " + testcol2.normal.x + ", y: " + testcol2.normal.y );
    		System.out.println("backtrack\n x: " + testcol2.backtrack.x + ", y: " + testcol2.backtrack.y );
    		Line normalL2 = new Line(testcol2.loc.getX(), testcol2.loc.getY(), testcol2.normal);
    		normalL2.color[0] = 1f;
    		normalL2.color[1] = .5f;
    		normalL2.color[2] = 1f;
    		testLines.add(normalL2);
    		Line backTrackL2 = new Line(testcol2.vertex.getX(), testcol2.vertex.getY(), testcol2.backtrack);
    		backTrackL2.color[0] = 1f;
    		backTrackL2.color[1] = 1f;
    		backTrackL2.color[2] = .2f;
